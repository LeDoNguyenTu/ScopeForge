...
import { buildNucleiExecutionPlan } from "./execution-plan";
import { NUCLEI_INITIAL_TEMPLATE_ID } from "./runtime-config";
...
function severityFromInfo(value: unknown): NucleiSeverity {
  const info = asRecord(value);
  const severity = boundedText(info.severity, 16);
  if (!severity || !SEVERITIES.has(severity as NucleiSeverity)) {
    throw new Error("NUCLEI_OUTPUT_SEVERITY_INVALID");
  }
  return severity as NucleiSeverity;
}

function parseLines(stdout: string): Array<{ event: Record<string, unknown>; digest: string }> {
  if (Buffer.byteLength(stdout, "utf8") > MAX_OUTPUT_BYTES) {
    throw new Error("NUCLEI_OUTPUT_LIMIT_EXCEEDED");
  }
...
export async function executeNucleiRunner(
  ...
  const plan = buildNucleiExecutionPlan(input.request, input.trustedHostname);
  const result = await (dependencies.driver ?? createDriver()).exec(
    NUCLEI_BINARY,
    plan.args,
    {
      timeoutMs: Math.min(8_000, input.context.maxRuntimeMs),
  ...
  const matches = parseLines(result.stdout).map(({ event, digest }) => {
    const templateId = boundedText(event["template-id"], 160);
    if (templateId !== NUCLEI_INITIAL_TEMPLATE_ID) {
      throw new Error("NUCLEI_OUTPUT_TEMPLATE_NOT_APPROVED");
    }
    const timestamp = boundedText(event.timestamp, 80);
    if (!timestamp || !Number.isFinite(Date.parse(timestamp))) {
      throw new Error("NUCLEI_OUTPUT_TIMESTAMP_INVALID");
    }
    if (event.interaction !== undefined
        || event.request !== undefined
        || event.response !== undefined
        || event["curl-command"] !== undefined
        || event["is_fuzzing_result"] === true) {
      throw new Error("NUCLEI_OUTPUT_FORBIDDEN_SURFACE_PRESENT");
    }
    return Object.freeze({
      targetNodeId: input.request.targetNodeId,
      templateId,
      severity: severityFromInfo(event.info),
      matcherName: boundedText(event["matcher-name"]),
