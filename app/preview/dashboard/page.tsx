...
export default async function DashboardPreview({ searchParams }: { searchParams: Promise<{ empty?: string }> }) {
  ...
  return <AppShell displayName="Preview user" workspaceName="Example workspace" role="preview" platformAdminHref="/preview/admin">
    <div className="saasPreviewNotice"><span><strong>Design preview</strong> · Sample data. Workspace actions require sign-in.</span><Link href={empty ? "/preview/dashboard" : "/preview/dashboard?empty=1"}>{empty ? "View populated state" : "View empty state"}</Link><Link href="/preview/security-runs">Security runs preview</Link><Link href="/auth/sign-in">Sign in to your workspace</Link></div>
    <ImmersiveDashboardExperience assets={assets} findings={findings} model={model} engine={{ recentRunCount: empty ? 0 : 2, operationalRuntimes: 1, validatingRuntimes: 2, enabledExternalRuntimes: 0, latestRun: empty ? null : { runId: "preview-run-accepted", status: "completed", stopReason: "request_budget_exhausted", updatedAt: "2026-09-24T07:42:00Z" } }} nextAction={{ href: empty ? "/dashboard/assets/new" : "/dashboard/assets", label: empty ? "Register asset" : "Review assets", title: empty ? "Register your first asset" : "Finish verifying your assets", copy: empty ? "Add an application, API, or repository that you control to start building your workspace." : "Documentation is still awaiting proof of control. Complete verification to prepare it for supported security workflows." }} />
  </AppShell>;
}
