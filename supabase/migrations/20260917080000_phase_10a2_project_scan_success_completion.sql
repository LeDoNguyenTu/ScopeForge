-- Complete connected-project orchestration after durable repository-scan
-- publication. Failed/cancelled scans keep the immutable snapshot for retry;
-- successful scans return the project control to idle.
...
begin
  ...
  if job_record.status = 'succeeded'::public.scan_job_status then
    ...
    return jsonb_build_object('reconciled', true, 'state', 'idle');
  end if;
...
-- Complete or release scans that reached terminal state before this repair.
do $$
declare
  pending record;
begin
