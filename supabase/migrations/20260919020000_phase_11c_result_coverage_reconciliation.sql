...
begin
  ...
  if counts_as_attempt then
    select array(
      ...
      from unnest(coverage_record.attempted_capability_ids || array[action_record.capability_id])
    ...
    select array(
      select node_id
      from unnest(coverage_record.untested_node_ids) as untested(node_id)
  ...
  next_request_count := coverage_record.request_count::bigint + target_request_count::bigint;
  next_provider_failure_count := coverage_record.provider_failure_count::bigint
    + case when target_status = 'provider_failed' then 1 else 0 end;

  if next_request_count > 2147483647 or next_provider_failure_count > 2147483647 then
...
begin
  ...
  set attempted_capability_ids = array(
      ...
      graph_expansion_count = greatest(current.graph_expansion_count, excluded.graph_expansion_count),
      provider_failure_count = greatest(current.provider_failure_count, excluded.provider_failure_count),
