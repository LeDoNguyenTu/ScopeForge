...
describe("passive runtime hosted finding integration", () => {
  it("uses the single passive result RPC instead of direct ledger or observation inserts", async () => {
    const source = await readFile(
      path.resolve(process.cwd(), "lib/runtime-observations/repository.ts"),
      "utf8",
    );
    ...
    expect(persistResult).toContain('admin.rpc("persist_passive_runtime_result"');
  ...
  it("persists the passive result before the success transition", async () => {
    const source = await readFile(
      path.resolve(process.cwd(), "lib/runtime-observations/service.ts"),
      "utf8",
    );
