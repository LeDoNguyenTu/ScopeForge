...
describe("Phase 5B security-remediation architecture", () => {
  ...
  it("enforces the exact passive runtime source version inside the retest-request transaction", async () => {
    const sql = await readFile(migrationPath, "utf8");
    const requestBody = functionBody(sql, "request_security_finding_retest");
    expect(requestBody).toMatch(
      /current_finding\.source_id\s*=\s*'scopeforge:runtime-observer'[\s\S]{0,180}current_finding\.source_version\s*=\s*'0\.1'/i,
    );
  });
