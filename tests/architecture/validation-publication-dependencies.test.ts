...
const packageDirectory = resolve(process.cwd(), "packages", "validation-publication");
...
describe("Phase 8C publication authority boundary", () => {
  it("keeps publication local/offline and outside hosted/runtime/UI authority", async () => {
  ...
  it("contains no dynamic execution, browser, network, or hosted capability activation", async () => {
    ...
    for (const file of await sourceFiles(packageDirectory)) {
      ...
      for (const flag of forbiddenCapabilityFlags) {
        expect(source, `${file}: ${flag}`).not.toContain(flag);
      }
    }
  });
});
