...
Status: approved architecture direction; Phase 11A implementation started after Phase 10A3 release
Date: 2026-09-17
Branch baseline: released post-Phase-10A3 `main` at `e493f1f339b8410793e94f61306715bfb05c6166`
...
- Phase 10A2 private repository acquisition is released.
- Phase 10A3 continuous GitHub scanning is released on `main`.
- Hosted runtime and scan capability flags remain separately gated.

Implementation of Phase 11 therefore follows these rules:

1. Pure domain and planner work proceeds from released post-10A3 `main`.
2. Phase 11 production migrations remain unapplied until the schema, ACL/RLS model, rollback path, and exact-candidate validation are reviewed.
3. Every new execution class remains default-off until separate operational acceptance.
4. No Phase 11 feature may widen an existing worker class in place when a new authority boundary is required.
5. Provider integrations remain capability-bound and removable; planner code never gains direct executable or network authority.
