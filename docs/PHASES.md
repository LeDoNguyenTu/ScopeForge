...
5. **Findings, Security Stories, and remediation - in progress**
   ...
   - **5B Security Stories, remediation, and deterministic retest - complete.** Adds workspace-scoped remediation assignment and notes, immutable retest source/profile snapshots, atomic `resolved -> retest_pending`, deterministic passive and bounded active-CORS retest orchestration through the existing runtime authorities, recovery for failed starts, authoritative fresh-evidence-only `verified_fixed`, bounded retest history, and deterministic Security Story v1. Merged through PR #33 as `eb35c2b23468addd817951486c60ac7d68710c9a` after exact-head CI #685 passed 148 test files / 654 tests plus typecheck, CLI build/smoke, benchmark, and production build.
   - **5C Hosted Phase 3 finding import - next design boundary.** Design a narrow trusted adapter for importing existing local/CI Phase 3 scanner results into the canonical hosted ledger. The design must define canonical identity, repository/asset binding, privacy and secret-redaction policy, evidence classifications, idempotent scan-run provenance, bounded payloads, and browser versus trusted-service authority. Do not route code findings through runtime-only ingestion contracts.
