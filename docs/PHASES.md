...
4. **Verified runtime and API security - in progress**
   ...
   - **4C-1 Bounded CORS origin-policy validation - implementation in PR #27, final gate pending.** The approved design merged through PR #26 as `3f0e46c61944976a4ddfd6ef039487498a19f839`. The implementation keeps `runtime-observer` passive, extracts shared low-level DNS/TLS/pinning/deadline behavior into `runtime-network`, and adds a separate `runtime-validator`. The only active profile is `cors-origin-policy@1`: owner/admin explicit authorization, one fixed synthetic-Origin unauthenticated GET to the exact verified HTTPS target, zero redirect following, zero request/response body capture, no cookies/credentials/user headers, fixed budgets, cancellation-safe trusted persistence, and deterministic `runtime_validated` findings. Broader active testing remains out of scope.
