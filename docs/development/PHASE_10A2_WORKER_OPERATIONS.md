...
The systemd unit provisions `/run/scopeforge-worker` with `RuntimeDirectory=scopeforge-worker`, preserves that shared directory while either template instance is restarted, and exposes it as `XDG_RUNTIME_DIR` with mode `0700`. Do not hardcode or depend on a numeric Linux UID or `/run/user/<uid>` path in the service definition. This keeps the deployment portable when the dedicated `scopeforge-worker` account receives a different UID on a rebuilt or replacement host and prevents one worker instance from invalidating the other instance's rootless Podman runtime state.
...
Stage `hosted-scanner-entry.js`, `main.wasm.gz`, and `deploy/worker/Containerfile.scanner` in a private host build directory. The scanner bundle contains its JavaScript dependencies. The image places the separate WASM asset one directory above the fixed `/app/hosted-scanner-entry.js` path, matching the HCL parser's runtime resolution without changing the sandbox entrypoint contract. The container source pins the accepted Node 24.16.0 base by registry digest. Build locally as the worker account, inspect the resulting digest, and set only the immutable result digest in the scan worker environment.
