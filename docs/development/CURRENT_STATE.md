...
- Local Node 24.16.0 focused pin checks, typecheck, CLI build/version and audit (0 vulnerabilities) passed. After the Windows portability fixes, the full run had 1,721 passed, 16 failed and 9 skipped; every remaining failure was an `EPERM` from this host being unable to create a test symlink. Linux CI remains necessary and must run the POSIX/rootless-Podman suites skipped on Windows. The portability changes normalize checkout newlines and path separators, allow realistic benchmark contention, and avoid asserting POSIX modes or commands on Windows without weakening their Linux coverage.
