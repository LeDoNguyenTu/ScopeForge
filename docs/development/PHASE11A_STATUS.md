...
- The registered worker has not heartbeated yet and no Phase 11 HTTP worker task has executed.
...
1. start the dedicated worker on the Oracle host using the accepted immutable runtime image
2. prove authenticated idle claim/heartbeat
3. prove class-scoped rollback
4. run one bounded authorized canary
