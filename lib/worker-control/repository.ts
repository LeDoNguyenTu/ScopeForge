...
function requiredString(value: unknown): string {
  if (typeof value !== "string" || value.length === 0) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return value;
}
...
function parseWorkerExecutionClass(value: unknown): WorkerControlExecutionClass {
  if (typeof value !== "string" || !ALL_EXECUTION_CLASSES.has(value as WorkerControlExecutionClass)) {
    throw new WorkerControlError("WORKER_CONTROL_FAILED");
  }
  return value as WorkerControlExecutionClass;
}
function mapRpcError(message: string): WorkerControlError {
  for (const code of KNOWN_CODES) if (message.includes(code)) return new WorkerControlError(code);
  return new WorkerControlError("WORKER_CONTROL_FAILED");
}
async function rpcData<T>(result: PromiseLike<{ data: T; error: { message: string } | null }>): Promise<T> {
...
function parseRegistration(value: unknown, expectedClass?: WorkerControlExecutionClass): WorkerRegistrationResult {
  const node = parseNode(value);
  if (expectedClass !== undefined && node.executionClass !== expectedClass) {
    throw new WorkerControlError("WORKER_CONTROL_FAILED");
  }
  return node;
}
function parseDisable(value: unknown): WorkerDisableResult {
  if (!isRecord(value)) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({ workerId: requiredString(value.workerId), disabledAt: requiredString(value.disabledAt) });
}
...
function parseRuntimeEnqueue(value: unknown, expectedClass: RuntimeWorkerExecutionClass): RuntimeWorkerEnqueueResult {
  if (!isRecord(value) || value.executionClass !== expectedClass) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({
...
function parseBudget(value: unknown, executionClass: WorkerControlExecutionClass): WorkerExecutionBudget {
  if (!isRecord(value)) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  const expected = profileBudget(executionClass);
  for (const [key, expectedValue] of Object.entries(expected)) {
    if (value[key] !== expectedValue) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  }
  if (Object.keys(value).length !== Object.keys(expected).length) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return expected;
}
...
function parseRepositoryScanInput(input: Record<string, unknown>) {
  if (input.kind !== "phase3_repository_scan") throw new WorkerControlError("WORKER_CONTROL_FAILED");
  const canonicalRepositoryUrl = requiredString(input.canonicalRepositoryUrl);
  let canonicalUrl: URL;
  try { canonicalUrl = new URL(canonicalRepositoryUrl); } catch { throw new WorkerControlError("WORKER_CONTROL_FAILED"); }
  if (canonicalUrl.protocol !== "https:" || canonicalUrl.hostname !== "github.com"
      || canonicalUrl.username !== "" || canonicalUrl.password !== "" || canonicalUrl.search !== ""
      || canonicalUrl.hash !== "" || canonicalUrl.pathname.split("/").filter(Boolean).length !== 2) {
    throw new WorkerControlError("WORKER_CONTROL_FAILED");
  }
  ...
  if (!COMMIT_PATTERN.test(resolvedCommitSha) || !DIGEST_PATTERN.test(contentDigest)
      || !DIGEST_PATTERN.test(artifactDigest) || input.scannerProfileId !== "phase3-hosted-static-v1"
      || input.scannerProfileVersion !== 1) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({
    kind: "phase3_repository_scan" as const,
    snapshotId: requiredUuid(input.snapshotId), canonicalRepositoryUrl, resolvedCommitSha,
    ...
    scannerProfileId: "phase3-hosted-static-v1" as const, scannerProfileVersion: 1 as const,
  });
}
function parseClaim(value: unknown): WorkerPersistenceClaimResult {
  if (value === null) return null;
  if (!isRecord(value) || !isRecord(value.input)) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  const executionClass = parsePersistenceExecutionClass(value.executionClass);
  const common = {
    taskId: requiredString(value.taskId), attemptId: requiredString(value.attemptId),
    leaseToken: requiredString(value.leaseToken), absoluteDeadlineAt: requiredString(value.absoluteDeadlineAt),
    budget: parseBudget(value.budget, executionClass),
  };
  if (!/^[a-f0-9]{64}$/.test(common.leaseToken)) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  if (executionClass === "foundation_no_egress_v1") {
    if (value.input.kind !== "foundation_probe") throw new WorkerControlError("WORKER_CONTROL_FAILED");
    return Object.freeze({ ...common, executionClass, input: Object.freeze({ kind: "foundation_probe" as const, nonce: requiredString(value.input.nonce) }) });
  }
  if (executionClass === "phase3_repository_scan_no_egress_v1") {
    return Object.freeze({ ...common, executionClass, input: parseRepositoryScanInput(value.input) });
  }
  ...
  if (value.input.kind !== "repository_snapshot_github_public") throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({
    ...common, executionClass, artifactObjectKey,
    input: Object.freeze({ kind: "repository_snapshot_github_public" as const, ...identity }),
  });
}
function parseRuntimeClaim(value: unknown): RuntimeWorkerPersistenceClaimResult {
  ...
  if (!hasExactKeys(value.input, ["kind", "domainJobId"])) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  ...
  const common = {
    taskId: requiredUuid(value.taskId), attemptId: requiredUuid(value.attemptId), leaseToken,
    absoluteDeadlineAt: requiredString(value.absoluteDeadlineAt), budget: parseBudget(value.budget, executionClass),
  };
  if (executionClass === "passive_runtime_observation_v1") {
    if (value.input.kind !== "passive_runtime_observation") throw new WorkerControlError("WORKER_CONTROL_FAILED");
    return Object.freeze({ ...common, executionClass, input: Object.freeze({ kind: "passive_runtime_observation" as const, domainJobId }) });
  }
...
function parseFinalization(value: unknown): WorkerFinalizationResult {
  if (!isRecord(value) || !["succeeded", "failed", "cancelled"].includes(String(value.outcome)) || typeof value.replayed !== "boolean") {
    throw new WorkerControlError("WORKER_CONTROL_FAILED");
  }
  return Object.freeze({
    taskId: requiredString(value.taskId), attemptId: requiredString(value.attemptId),
    outcome: value.outcome as WorkerFinalizationResult["outcome"], replayed: value.replayed,
  });
}
function parseFleetNode(value: unknown): WorkerFleetNodeSnapshot {
  if (!isRecord(value)) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({
    workerId: requiredString(value.workerId), executionClass: parseWorkerExecutionClass(value.executionClass),
    softwareVersion: requiredString(value.softwareVersion), registeredAt: requiredString(value.registeredAt),
    lastSeenAt: nullableString(value.lastSeenAt), disabledAt: nullableString(value.disabledAt),
  });
}
function parseFleetTaskCounts(value: unknown): WorkerFleetTaskCounts {
  if (!isRecord(value)) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({
    queued: boundedCount(value.queued), leased: boundedCount(value.leased), retryWait: boundedCount(value.retryWait),
    completed: boundedCount(value.completed), deadLetter: boundedCount(value.deadLetter), cancelled: boundedCount(value.cancelled),
  });
}
function parseRuntimeFleetClass<TExecutionClass extends RuntimeWorkerExecutionClass>(
  value: unknown, expectedExecutionClass: TExecutionClass, expectedCapacity: number, activeLeaseCount: number,
) {
  const expectedKeys = ["executionClass", "enabledNodeCount", "leasedCount", "capacity", "available", "saturated"] as const;
  if (!isRecord(value) || !hasExactKeys(value, expectedKeys) || value.executionClass !== expectedExecutionClass || value.capacity !== expectedCapacity) {
    throw new WorkerControlError("WORKER_CONTROL_FAILED");
  }
  ...
  if (value.available !== available || value.saturated !== saturated) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({ executionClass: expectedExecutionClass, enabledNodeCount, leasedCount, capacity: expectedCapacity, available, saturated });
}
function parseRuntimeFleetHealth(value: unknown, activeLeaseCount: number): WorkerFleetSnapshot["runtimeClasses"] {
  if (!isRecord(value) || !hasExactKeys(value, ["passiveRuntime", "activeCors"])) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  return Object.freeze({
    passiveRuntime: parseRuntimeFleetClass(value.passiveRuntime, "passive_runtime_observation_v1", 2, activeLeaseCount),
    activeCors: parseRuntimeFleetClass(value.activeCors, "active_cors_validation_v1", 1, activeLeaseCount),
  });
}
function parseFleetSnapshot(value: unknown): WorkerFleetSnapshot {
  if (!isRecord(value) || !Array.isArray(value.nodes) || value.nodes.length > 100) throw new WorkerControlError("WORKER_CONTROL_FAILED");
  const activeLeaseCount = boundedCount(value.activeLeaseCount);
  return Object.freeze({
    generatedAt: requiredString(value.generatedAt), nodes: Object.freeze(value.nodes.map(parseFleetNode)),
    taskCounts: parseFleetTaskCounts(value.taskCounts), activeLeaseCount,
...
export interface WorkerControlRepository {
  ...
  registerPrivateRepositorySnapshot(input: WorkerRegistrationInput): Promise<WorkerRegistrationResult>;
...
export function createWorkerControlRepository(
  ...
  return Object.freeze<CompleteWorkerControlRepository>({
    ...
    async registerPrivateRepositorySnapshot(input) {
      return parseRegistration(await rpcData(client.rpc("register_private_repository_snapshot_worker_node", {
        target_credential_hash: input.credentialHash, target_software_version: input.softwareVersion,
      })), "repository_snapshot_github_private_v1");
    },
    ...
    async registerPassiveRuntime(input) {
      return parseRegistration(await rpcData(client.rpc("register_passive_runtime_worker_node", {
        target_credential_hash: input.credentialHash, target_software_version: input.softwareVersion,
      })), "passive_runtime_observation_v1");
    },
    ...
    async authenticate(input) {
      return parseNode(await rpcData(client.rpc("authenticate_worker_node", {
        target_worker_id: input.workerId, target_credential_hash: input.credentialHash,
      })));
    },
    async enqueueFoundationProbe(input) {
      return parseFoundationProbe(await rpcData(client.rpc("enqueue_foundation_worker_task", {
        target_workspace_id: input.workspaceId, target_asset_id: input.assetId, target_actor_id: input.actorId,
      })));
    },
    async enqueuePassiveRuntime(input) {
      return parseRuntimeEnqueue(await rpcData(client.rpc("enqueue_passive_runtime_worker_task", {
        target_workspace_id: input.workspaceId, target_scan_job_id: input.scanJobId, target_actor_id: input.actorId,
      })), "passive_runtime_observation_v1");
    },
    ...
    async claimRepositoryScan(input) {
      const claim = parseClaim(await rpcData(client.rpc("claim_repository_scan_worker_task", { target_worker_id: input.workerId })));
      if (claim !== null && claim.executionClass !== "phase3_repository_scan_no_egress_v1") throw new WorkerControlError("WORKER_CONTROL_FAILED");
      return claim;
    },
    ...
    async heartbeat(input) {
      return parseHeartbeat(await rpcData(client.rpc("heartbeat_worker_attempt", {
        target_worker_id: input.workerId, target_task_id: input.taskId,
        target_attempt_id: input.attemptId, target_lease_token: input.leaseToken,
      })));
    },
    async finalize(input) {
      return parseFinalization(await rpcData(client.rpc("finalize_worker_attempt", {
        target_worker_id: input.workerId, target_task_id: input.taskId,
        target_attempt_id: input.attemptId, target_lease_token: input.leaseToken,
        target_terminal_outcome: input.terminalOutcome, target_failure_code: input.failureCode,
        target_terminal_payload_digest: input.terminalPayloadDigest,
        target_wall_time_ms: input.wallTimeMs, target_cpu_time_ms: input.cpuTimeMs,
        target_peak_memory_bytes: input.peakMemoryBytes, target_input_bytes: input.inputBytes,
